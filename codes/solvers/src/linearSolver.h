...
double Dot(VD &vec1, VD &vec2 , mpiInfo &myMPI)
{
  ...
  rLOOP sum += vec1[r]*vec2[r]; /* TO-DO in Lab: Correct this line to not double-count on PE boundaries */

  /* TO-DO in Lab: Ensure each PE has the full dot product value */
...
void MatVecProd(VD &p , VD &prod , mpiInfo &myMPI)
{
  ...
  /* TO-DO in Lab: Ensure the matrix-vector product is correct on PE boundaries */
...
void CG(VD &Solution , mpiInfo & myMPI)
{
  ...
  // (2.1) Create a vector that can be used to sum the RHS vector, b[],
  //       on PE boundaries.
  ...
  while ( global_converged == 0  && ++iter <= max_iter)
    {
      ...
      alpha    = /* TO-DO in Lab */       // alpha, as computed on the Wikipedia page
      ...
      // (4.3) Compute beta (The form used here differs from the Wikipedia page but is still correct.)
